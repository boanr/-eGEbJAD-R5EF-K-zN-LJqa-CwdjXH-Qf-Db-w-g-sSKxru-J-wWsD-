...
WITH WPH_LIST AS (
	SELECT WFR_QTY,
	CASE
	WHEN ((EQP_ID='AETET301' AND CHAMBER='C') OR (EQP_ID='AETET302' AND CHAMBER='C')) AND LOT_TYPE='MD' THEN 'MD'
	WHEN STEP_NAME LIKE 'ET-ILD-Oxide_Etch%' THEN 'ILD'
	WHEN STEP_NAME LIKE 'ET-CVD-Oxide_Etch%' THEN 'CVD'
	WHEN STEP_NAME LIKE 'ET-ZC-Oxide_Etch%' THEN 'ZC'
	END DEVICE_ID
	FROM CIMDM.DM_MOVE_BT WHERE (STEP_NAME LIKE 'ET-ZC-Oxide_Etch%' OR STEP_NAME LIKE 'ET-ILD-Oxide_Etch%' OR STEP_NAME LIKE 'ET-CVD-Oxide_Etch%' OR LOT_TYPE='MD')  AND ((EQP_ID='BETAM001' AND CHAMBER='C') OR (EQP_ID='AETET301' AND CHAMBER='C') OR (EQP_ID='AETET302' AND CHAMBER='C'))
...
WPH_TABLE AS (
	...
	WHEN DEVICE_ID='ILD' THEN 12.6
	WHEN DEVICE_ID='CVD' THEN 6.8
	WHEN DEVICE_ID='ZC'  THEN 6.8
    WHEN DEVICE_ID='MD'  THEN 6.67
...
UP_LIST AS (
	SELECT ROUND(AVG(UP),4) UP FROM CIMDM.RPT_KER_SUM WHERE (EQP_ID IN ('AETET301','AETET302') AND FAB='FAB1')
    AND WIPDATE = TO_DATE(SYSDATE-1,'yyyy-mm-dd')
),
AVL_LIST AS (
	SELECT ROUND(AVG(UP+LOST),4) UP FROM CIMDM.RPT_KER_SUM WHERE (EQP_ID IN ('BETAM001','AETET301','AETET302') AND FAB='FAB1')
	AND WIPDATE = TO_DATE(SYSDATE-1,'yyyy-mm-dd')
)
